...
export class TarefaService {
  ...
  listarTodas(): Observable<Tarefa[]> {
    return this.http.get<Tarefa[]>(`${this.API}?usuarioId=${this.usuarioId}`);
  }

  listarPorProjeto(projetoId: number): Observable<Tarefa[]> {
    return this.http.get<Tarefa[]>(`${this.API}/projeto/${projetoId}?usuarioId=${this.usuarioId}`);
  }

  criar(request: TarefaRequest): Observable<Tarefa> {
    return this.http.post<Tarefa>(`${this.API}?usuarioId=${this.usuarioId}`, request);
  }

  atualizar(id: number, request: TarefaRequest): Observable<Tarefa> {
    return this.http.put<Tarefa>(`${this.API}/${id}?usuarioId=${this.usuarioId}`, request);
  }

  deletar(id: number): Observable<void> {
    return this.http.delete<void>(`${this.API}/${id}?usuarioId=${this.usuarioId}`);
  }
}