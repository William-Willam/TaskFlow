...
export class ProjetoService {
  ...
  listar(): Observable<Projeto[]> {
    return this.http.get<Projeto[]>(`${this.API}?usuarioId=${this.usuarioId}`);
  }

  criar(request: ProjetoRequest): Observable<Projeto> {
    return this.http.post<Projeto>(`${this.API}?usuarioId=${this.usuarioId}`, request);
  }

  atualizar(id: number, request: ProjetoRequest): Observable<Projeto> {
    return this.http.put<Projeto>(`${this.API}/${id}?usuarioId=${this.usuarioId}`, request);
  }
